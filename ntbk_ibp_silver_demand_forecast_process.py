...
silver_df = src_df2.select(col('DMNDFCST_GNRTN_DTM').alias('FCST_EXCTN_DT'),
               ...
               col('DMNDFCST_MKT_UNIT_CDV').alias('MKT_UNIT'),
...
silver_df = silver_df.withColumn("WEEK_OF_YEAR",concat(when((weekofyear(col("FCST_DT")) == 1) & (month(col("FCST_DT")) == 12),(year(col("FCST_DT"))+1)).otherwise(year(col("FCST_DT"))),when(weekofyear(col("FCST_DT"))<10, concat(lit(0),weekofyear(col("FCST_DT")))).otherwise(weekofyear(col("FCST_DT")))))
display(silver_df)

# COMMAND ----------

silver_df = silver_df.withColumn("PROCESS_DATE",current_timestamp()).withColumn("WEEK_OF_YEAR",silver_df.WEEK_OF_YEAR.cast('int'))
...
DeltaTable.createIfNotExists(spark) \
    .tableName("sc_ibp_silver.dfu_to_sku_forecast") \
    .location(tgtPath) \
    .execute()
...
# MAGIC %sql
# MAGIC OPTIMIZE sc_ibp_silver.dfu_to_sku_forecast
