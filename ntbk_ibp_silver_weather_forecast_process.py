...
#Reading the weather forecast data from source table - weather_forecast_onpoint
weather_fcst = spark.sql("select * from "+srcPath)
display(weather_fcst)

# COMMAND ----------

#Converting the columns time_init_utc and date_valid_std to date type
weather_fcst = weather_fcst.withColumn("time_init_utc",to_timestamp("time_init_utc", 'yyyy-MM-dd HH:mm:ss')).withColumn("date_valid_std",to_date("date_valid_std", 'yyyy-MM-dd'))
display(weather_fcst)

# COMMAND ----------

#Max and Min date_valid_std in the source data
min_source_date = weather_fcst.agg({"time_init_utc": "min"}).collect()[0][0]
max_source_date = weather_fcst.agg({"time_init_utc": "max"}).collect()[0][0]
...
if DeltaTable.isDeltaTable(spark, tgtPath):
  ...
  max_silver_date = temp_df.agg({"time_init_utc": "max"}).collect()[0][0]
  print("The maximum date value present in silver layer is: "+str(max_silver_date))
  weather_fcst_fltrd = weather_fcst.filter(weather_fcst.time_init_utc > max_silver_date)
  print("Full Count: "+str(weather_fcst.count()))
  print("Incremental Count: "+str(weather_fcst_fltrd.count()))
  #source_df will be used in all subsequent transformations
  source_df = weather_fcst_fltrd
...
# silver_df_final = silver_df.select("RGN","PRVN","MU","TIME_INIT_UTC","DT",col("D_YR").cast('int').alias("D_YR"),"LOC","MU_CHNL","CTGY", "WGHTD_AVG_TMP_AIR_2M_F","WGHTD_AVG_TMP_WTBLB_2M_F",'WGHTD_AVG_TMP_DWPNT_2M_F','WGHTD_AVG_TMP_FLSLK_2M_F','WGHTD_AVG_TMP_WNDCHLL_2M_F','WGHTD_AVG_TMP_HTNDX_2M_F','WGHTD_AVG_HMDTY_RLTV_2M_PCT','WGHTD_AVG_HMDTY_SPEC_2M_GPKG','WGHTD_AVG_PRSSR_2M_MB','WGHTD_AVG_PRSSR_MEAN_SEA_LEVEL_MB','WGHTD_AVG_WIND_SPD_10M_MPH','WGHTD_AVG_WIND_DRCTN_10M_DEG','WGHTD_AVG_WIND_SPD_80M_MPH','WGHTD_AVG_WIND_DRCTN_80M_DEG','WGHTD_AVG_WIND_SPD_100M_MPH','WGHTD_AVG_WIND_DRCTN_100M_DEG','WGHTD_TOT_PRECIPITATION_IN','WGHTD_TOT_SNWFALL_IN','WGHTD_AVG_CLD_CVR_TOT_PCT','WGHTD_AVG_RAD_SOLAR_TOTAL_WPM2').distinct()
...
#Filtering out records having NULL values for date columns - TIME_INIT_UTC; DT
silver_df_final = silver_df_final.filter("DT is not NULL").filter("TIME_INIT_UTC is not NULL")
...
DeltaTable.createIfNotExists(spark) \
    .tableName("sc_ibp_silver.weather_forecast") \
    .location(tgtPath) \
    .execute()
...
# MAGIC %sql
# MAGIC OPTIMIZE sc_ibp_silver.weather_forecast
