...
## Historical lookup tables that will serve as reference
...
# DBTITLE 1,Aggregating by Week or Month (ie, Year Agnostic) - FOR HISTORICAL REFERENCE
...
## Creating aggregation dictionaries
mean_cols_week = list(set(weather_weekly_hist.columns) - set(["country", "Week_Of_Year", "Year_Only", "Week_Only"]))
mean_cols_month = list(set(weather_monthly_hist.columns) - set(["country", "Month_Of_Year", "Year_Only", "Month_Only"]))

mean_dict_week = {x: 'mean' for x in mean_cols_week}
mean_dict_month = {x: 'mean' for x in mean_cols_month}

## Checks on the above
print(len(mean_cols_week) == len(mean_dict_week))
print(len(mean_cols_month) == len(mean_dict_month))

# COMMAND ----------

## Aggregated using 'custom' function
weather_weekly_hist = timely_agg(weather_weekly_hist, mean_dict_week, mean_cols_week, country_col="country", time_var="Week_Only")
weather_monthly_hist = timely_agg(weather_monthly_hist, mean_dict_month, mean_cols_month, country_col="country", time_var="Month_Only")

for col_name in mean_cols_week:
  weather_weekly_hist = weather_weekly_hist.withColumn(col_name, round(col_name, 3))
  
for col_name in mean_cols_month:
  weather_monthly_hist = weather_monthly_hist.withColumn(col_name, round(col_name, 3))

# COMMAND ----------

display(weather_weekly_hist)

## Miscellaneous Column Stats
weather_weekly_hist.select('weather_count_heat').describe().show()
...
## Miscellaneous Column Stats
weather_monthly_hist.select('weather_count_heat').describe().show()

# COMMAND ----------

# DBTITLE 1,Save as Delta Tables
...
weather_weekly_hist.toPandas().plot(y=['weather_tavg', 'weather_tmin', 'weather_tmax'], kind='line')
plt.show()

# COMMAND ----------

weather_monthly_hist.toPandas().plot(y=['weather_tavg', 'weather_tmin', 'weather_tmax'], kind='line')
plt.show()

# COMMAND ----------

## Weekly QC check - plug in various variable names
weekly_plot_pd = weather_weekly_hist.toPandas()
print(weekly_plot_pd.columns)
sns.barplot(x='Week_Only', y='weather_tmin', hue='country', data=weekly_plot_pd)
plt.xticks(rotation=90)
plt.show()

# COMMAND ----------

## Monthly QC check - plug in various variable names
monthly_plot_pd = weather_monthly_hist.toPandas()
print(monthly_plot_pd.columns)
sns.barplot(x='Month_Only', y='weather_increase_count_prcp', hue='country', data=monthly_plot_pd)
plt.show()
