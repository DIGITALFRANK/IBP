...
for path in dependentPath_list:
  ...
  if '/alteryx/ibp-poc/product-master' in path:
...
CTE_df = udtMaterialCountryDF.select(udtMaterialCountryDF.EJDA_MTRL_ID.alias('ITEM'), udtMaterialCountryDF.MTRL_XREF_ID.alias('MATERIAL'), udtMaterialCountryDF.MKT_CDV.alias('JDA_CODE'), udtMaterialCountryDF.APPROVED, udtMaterialCountryDF.MTRL_STTS_NM.alias('STATUS'), udtMaterialCountryDF.CTRY_CDV.alias('SOURCE_SYSTEM_COUNTRY'), udtMaterialCountryDF.MTRL_TYP_CDV)

display(CTE_df)   

# COMMAND ----------

CTE2_df_temp = CTE_df.filter(CTE_df.JDA_CODE.isin('04','05','06')).groupBy("ITEM", "MATERIAL", "JDA_CODE", "APPROVED", "STATUS", "MTRL_TYP_CDV").agg(countDistinct("SOURCE_SYSTEM_COUNTRY").alias("Source_System_Cnt"))

CTE2_df = CTE2_df_temp.select(CTE2_df_temp.ITEM, CTE2_df_temp.MATERIAL, CTE2_df_temp.JDA_CODE, CTE2_df_temp.APPROVED, CTE2_df_temp.STATUS, CTE2_df_temp.MTRL_TYP_CDV, CTE2_df_temp.Source_System_Cnt, row_number().over( Window.partitionBy("ITEM").orderBy(col("APPROVED").desc(), col("STATUS").desc())).alias("ROW_Id"))
...
materialCountry = CTE2_df.filter(CTE2_df.ROW_Id == 1).select(CTE2_df.ITEM, CTE2_df.MATERIAL, CTE2_df.JDA_CODE, CTE2_df.APPROVED, CTE2_df.STATUS, CTE2_df.MTRL_TYP_CDV, CTE2_df.Source_System_Cnt, CTE2_df.ROW_Id)
