...
"""
This notebook is used to perform validations on the Product Master data present in Silver layer
"""
...
path = "abfss://supplychain-ibp@cdodevadls2.dfs.core.windows.net/silver/iberia/ibp-poc/product-master"
...
##PK Null check
null_df = silver_df.filter("PROD_CD is NULL")
print("There are total of "+str(null_df.count())+" rows with NULL values on PK columns")

# COMMAND ----------

#PK Duplicate check
dup_df = silver_df.groupBy("PROD_CD").count().filter("count > 1")
display(dup_df)
...
display(silver_df.filter(silver_df.MOSAIC_SKU_CDV == "01-11026001"))
