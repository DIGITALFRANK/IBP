...
map_bev_deltaTable = DeltaTable.forPath(spark, map_ean_bev_path)
map_snacks_deltaTable = DeltaTable.forPath(spark, map_ean_snacks_path)
...
#Reading Shipment Data#Khong Update
def get_shipment():
...
mapping_pt_snacks_df = mapping_pt\
                          ...
                          .withColumn("WeekMaxDate", max(to_date(concat("WeekCol2", lit("-1")), "YYYY-'W'ww-u")).over(Window.partitionBy("LDESC_1","LDESC","TAG","PLANG_MTRL_GRP_VAL","ean_code")))\
...
mapping_sp_snacks_df = mapping_es_snacks\
                          ...
                          .withColumn("WeekMaxDate", max(to_date(concat("WeekCol2", lit("-1")), "YYYY-'W'ww-u")).over(Window.partitionBy("LDESC","SDESC","TAG","PLANG_MTRL_GRP_VAL","ean_code")))\
                          .withColumn("Thresholdweek", add_months(col("WeekMaxDate"),-3))\
                          .withColumn("quartyear",year(col("Thresholdweek")))\
...
col_port_snacks = ['"PORTUGAL" as CNTRY_SRC_NM','NULL as AROMA','Distribuio_Numrica_S as NUMRC_DSTRBTN','NULL AS NUMRC_DSTRBTN_PROMO','Distribuio_Ponderada_S as WGHTD_DSTRBTN','Distribuio_Ponderada_S_reduo_de_preo as WGHTD_DSTRBTN_PRC_RDCTN','FABRICANTE AS MANUFACTURER','LDESC_1 as LDESC1','MARCA as BRAND','NULL AS WEIGHT','NULL AS PRC_PER_QTY','NULL AS PRMTNL_PRC_PER_LT_KG','PROCESS_DATE as PROCESS_DATE','SABORES as FLAVOR','LDESC AS SDESC','SEGMENTO AS SEGMENT','NULL AS SEGMENTS_GTC','TAG AS TAG','NULL AS PRODUCT_TYPE_VARIETY','NULL AS UNIVERSE','Vendas_em_Unidades as UNITS_SLS','Vendas_em_Valor as VAL_SLS','Vendas_Base_em_Valor as BS_SLS_VAL','Vendas_em_Valor_com_promoo as VAL_SLS_PROMO','Vendas_em_Valor_reduo_de_preo as Ventas_en_valor_reduccion_temp_de_prec','Vendas_Base_em_Quantidade as BS_SLS_QTY','WEEK as WEEK','ean_code as EAN_CODE','PLANG_MTRL_GRP_VAL as PROD_CD','weekVal','WeekCol','QuartweekVal','"PT" as MU']

col_spain_snacks = ['"SPAIN" as CNTRY_SRC_NM','AROMA as AROMA','Distribucion_numerica_S  as NUMRC_DSTRBTN','Distribucion_numerica_S_en_promocion AS NUMRC_DSTRBTN_PROMO','Distribucion_ponderada_S  as WGHTD_DSTRBTN','Distribucion_ponderada_S_reduccion_temp_de_prec  as WGHTD_DSTRBTN_PRC_RDCTN','FABRICANTES  as MANUFACTURER','LDESC  as LDESC1','MARCAS  as BRAND','PESOS AS WEIGHT','Precio  as PRC_PER_QTY','Precio_en_promocion  as PRMTNL_PRC_PER_LT_KG','PROCESS_DATE AS PROCESS_DATE','SABOR  as FLAVOR','SDESC  as SDESC','SEGMENTOS_GTC  as SEGMENT','NULL AS SEGMENTS_GTC','TAG  as TAG','TIPO_DE_PRODUCTO_VARIEDAD AS PRODUCT_TYPE_VARIETY','Universo AS UNIVERSE','Ventas_en_unidades  as UNITS_SLS','Ventas_en_valor  as VAL_SLS','Ventas_en_valor_baseline  as BS_SLS_VAL','Ventas_en_valor_en_promocion AS VAL_SLS_PROMO','Ventas_en_valor_reduccion_temp_de_prec as Ventas_en_valor_reduccion_temp_de_prec','Ventas_unidades_equivalentes_baseline  as BS_SLS_QTY','WEEK  as WEEK','ean_code as EAN_CODE','PLANG_MTRL_GRP_VAL as PROD_CD','weekVal','WeekCol','QuartweekVal','"ES" as MU']

col_spain_bev = ['"BEVERAGES" as CNTRY_SRC_NM','NULL as AROMA','Distribucion_numerica_S as NUMRC_DSTRBTN','Distribucion_numerica_S_en_promocion as NUMRC_DSTRBTN_PROMO','Distribucion_ponderada_S AS WGHTD_DSTRBTN','Distribucion_ponderada_S_reduccion_te as WGHTD_DSTRBTN_PRC_RDCTN','NULL as MANUFACTURER','LDESC as LDESC1','MARCA as BRAND','NULL AS WEIGHT','Precio  as PRC_PER_QTY','Precio_en_promocion  as PRMTNL_PRC_PER_LT_KG','PROCESS_DATE AS PROCESS_DATE','SABORES as FLAVOR','SDESC  as SDESC','SEGMENTOS as SEGMENT','SEGMENTOS_GTC as SEGMENTS_GTC','NULL AS TAG','NULL AS PRODUCT_TYPE_VARIETY','Universo as UNIVERSE','Ventas_en_unidades AS UNITS_SLS','Ventas_en_valor as VAL_SLS','Ventas_en_valor_baseline  as BS_SLS_VAL','Ventas_en_valor_en_promocion AS VAL_SLS_PROMO','Ventas_en_valor_reduccion_temp_de_prec as Ventas_en_valor_reduccion_temp_de_prec','Ventas_unidades_equivalentes_baseline as BS_SLS_QTY','WEEK as WEEK','ean_code as EAN_CODE','PLANG_MTRL_GRP_VAL as PROD_CD','weekVal','WeekCol','QuartweekVal','"ES" as MU']
...
nielsen_hist_union_df = mapping_pt_snacks_df2.union(mapping_sp_snacks_df2).union(mapping_sp_beverages_df2)

# COMMAND ----------

nielsen_hist_union_df2= nielsen_hist_union_df.fillna('0',subset=['VAL_SLS','VAL_SLS_PROMO','WGHTD_DSTRBTN','NUMRC_DSTRBTN','UNITS_SLS'])

# COMMAND ----------

display(nielsen_hist_union_df2.where(col("PROD_CD").isNotNull()).groupBy(col("SOURCE"),col("SDESC"),col("PROD_CD"),col("EAN_CODE"),col("LDESC1"),col("TAG"),col("WEEK"))\
       .agg(count(col('TAG')).alias("DUP_CHECK"))\
...
spark.sql("set spark.sql.legacy.timeParserPolicy=LEGACY")
silverr_df = nielsen_hist_union_df2.withColumn("CUST_GRP",when(col("SDESC")=='TOTAL ALCAMPO','ES_OT_CARREFOUR')\
