...
df = df \
  .withColumn("Week_end_date",to_timestamp(date_add("Week_start_date",6)))\
...
calendar=df.groupBy("Month_Of_Year_WSD","Month_Of_Year","Week_Of_Year","Week_start_date","Week_end_date").agg(F.sum("flag").cast("int").alias("Day_count"))\
                    .withColumn("month_ratio",(col("Day_count")/lit(7)).cast('float'))\
                    .withColumn("week_ratio",lit(1).cast('float'))
display(calendar)
...
calendar = calendar.withColumn("PROCESS_DATE",current_timestamp())
...
DeltaTable.createIfNotExists(spark) \
    .tableName("sc_ibp_silver.time_master") \
    .location(tgtPath) \
    .execute()
...
# MAGIC %sql
# MAGIC OPTIMIZE sc_ibp_silver.time_master
