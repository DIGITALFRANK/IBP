...
silver_df = stft_dq_pass.select(stft_dq_pass.country.alias("CTRY")
               ...
               ,stft_dq_pass.latitude.cast("float").alias("LAT")
               ,stft_dq_pass.longitude.cast("float").alias("LNGTD")
               ,stft_dq_pass.rating.cast("float").alias("RTNG")
               ,stft_dq_pass.reviews.cast("int").alias("RVWS")
               ,stft_dq_pass.hours.alias("HRS")
               ,stft_dq_pass.price.cast("int").alias("PRC")
               ,stft_dq_pass.does_shop_do_delivery.cast("int").alias("SHOP_DLVRY")
               ,stft_dq_pass.store_id.alias("STOR_ID")
               ,stft_dq_pass.TOTAL_VISITORS_SUM_7_DAYS.cast("int").alias("TOT_VISTORS_7_DAYS")
               ,stft_dq_pass.TOTAL_VISITORS_SUM_14_DAYS.cast("int").alias("TOT_VISTORS_14_DAYS")
               ,stft_dq_pass.AWAKENING_SCORE.cast("float").alias("AWAKENING_SCR")
               ,stft_dq_pass.PERCENT_CHANGE_FROM_BENCHMARK.cast("float").alias("PCT_CHNG_FROM_BNCHMRK")
               ,stft_dq_pass.PERCENT_CHANGE_FROM_BENCHMARK_AVG_7_DAYS.cast("float").alias("PCT_CHNG_FROM_BNCHMRK_AVG_7_DAYS")
...
DeltaTable.createIfNotExists(spark) \
    .tableName("sc_ibp_silver.store_foot_traffic") \
    .location(tgtPath) \
    .execute()
...
# MAGIC %sql
# MAGIC OPTIMIZE sc_ibp_silver.store_foot_traffic
