...
silver_df = fillRateCSL_df.filter('Market in ("ES","PT") and Metric_Numerator is not null and Metric_Numerator is not null ') \
               .withColumn("CUST_GRP",when(trim(col("Customer_ID")) == 'Total',col("Market")).otherwise(col("Customer_ID"))) \
               .withColumn("SHPPED",regexp_replace(col("Metric_Numerator"), ",", ""))\
...
DeltaTable.createIfNotExists(spark) \
    .tableName("sc_ibp_silver.fillrate_customer_service_levels") \
    .location(tgtPath) \
    .execute()
...
# MAGIC %sql
# MAGIC OPTIMIZE sc_ibp_silver.fillrate_customer_service_levels
