...
silver_df = aop_planning_df_filtered.join(product_df, prod_cond, "inner").join(customer_df, cust_cond, "inner").join(aop_financial_df, fin_cond, "inner").join(aop_channel_df, chan_cond, "inner").select(product_df.BRND_CDV.alias('BRND_CDV'),
...
DeltaTable.createIfNotExists(spark) \
    .tableName("sc_ibp_silver.finance") \
    .location(tgtPath) \
    .execute()
...
# MAGIC %sql
# MAGIC OPTIMIZE sc_ibp_silver.finance
